...
import dk.aau.cs.qweb.piqnic.bloom.hash.IHashFunction;
import dk.aau.cs.qweb.piqnic.bloom.hash.Murmur3;
...
public class PrefixPartitionedBloomFilter implements IBloomFilter<String> {
    ...
    @Expose
    private PredicateSpecificMetaFragment fragment;
    ...
    private PrefixPartitionedBloomFilter(long numElements, double maxFpp, String file) {
        ...
        if (!f.exists() || !f1.exists()) {
            ...
            //System.out.println("File doesn't exist. Building.");
            try {
                f.createNewFile();
                f1.createNewFile();
            } catch (IOException e) {
    ...
    public PredicateSpecificMetaFragment getFragment() {
        return fragment;
    }

    public void setFragment(PredicateSpecificMetaFragment fragment) {
    ...
    public boolean mightContain(String element) {
        ...
        //String prefix = element.substring(0, element.lastIndexOf("/"));
        if (!partitions.containsKey(prefix)) return false;

        long offset = partitions.get(prefix);
        //String name = element.substring(element.lastIndexOf("/") + 1);
    ...
    public IBloomFilter<String> intersect(IBloomFilter<String> other) {
        ...
        //File f2 = new File(ppbf.filename);
        String newFilename = "/q/storage/Aebeloe/ppbfs/" + filename.substring(filename.lastIndexOf("/") + 1).replace(".hdt.ppbf", "") + "-" + ppbf.filename.substring(ppbf.filename.lastIndexOf("/") + 1).replace(".hdt.ppbf", "") + ".ppbf";
        //String newFilename = f1.getParent() + "/" + getFileNameWithoutExtension(f1) + getFileNameWithoutExtension(f2) + "." + FilenameUtils.getExtension(f2.getAbsolutePath());
