...
public class IndexPeer implements IPeer {
    ...
    public IndexPeer(String address, int port, UUID id) {
        this.address = address;
        this.port = port;
        this.id = id;
    }

    public IndexPeer(PiqnicNode node) {
        this.address = node.getIp();
        this.port = node.getPort();
        this.id = node.getId();
    }

    public IndexPeer(Peer node) {
    ...
    private Peer toPeer() {
        return new Peer(address, port, id);
    }
    ...
    public Map<MetaFragmentBase, IBloomFilter<String>> getBloomsForFragments(Set<MetaFragmentBase> fragments) throws IOException {
        Socket socket;
        try {
            socket = new Socket(address, port);
        } catch (Exception e) {
        ...
        Gson gson = new Gson();
        Map<MetaFragmentBase, IBloomFilter<String>> ret = new HashMap<>();
        String response;
        while ((response = in.readLine()) != null) {
            if(response.equals("EOF")) break;
            PrefixPartitionedBloomFilter f = PrefixPartitionedBloomFilter.fromString(response);
            response = in.readLine();
            MetaFragmentBase fr = gson.fromJson(response, PredicateSpecificMetaFragment.class);
            ret.put(fr, f);
        }
        socket.close();
        return ret;
