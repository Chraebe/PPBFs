...
public class TestClient implements IClient {
    ...
    private void processConnection(PrintWriter writer) throws Exception {

        /*queries/S1 [flood|bind|down]*/

        //String[] words = line.split(" ");
        File outDir = new File("experiments/index/no");
        outDir.mkdirs();

        String warmup = new String(Files.readAllBytes(Paths.get("queries/S2")), StandardCharsets.UTF_8);
        File output = new File("experiments/index/no/warmup.txt");

        writer.println("Doing warmup...");
        process(warmup, output, false, 2);

        for (final File queryFile : new File("queries").listFiles()) {
            writer.println("Processing "+queryFile.getName());
            performTests(writer, queryFile);
        }


    }

    private void performTests(PrintWriter writer, File queryFile) throws Exception {
        File output = new File("experiments/index/no/" + queryFile.getName() + ".txt");
    ...
    private void process(String sparql, File output, boolean log, int delay) throws Exception {
        ...
        //final PiqnicExtGraph graph = new PiqnicExtGraph();
