...
public class Peer implements IPeer, Comparable<Peer> {
    ...
    public void addFragmentForTest(FragmentBase fragment, int ttl, PrintWriter writer) throws IOException {
        ...
        Peer o = fragment.getOwner();
        out.println(o.getAddress() + ";" + o.getPort() + ";" + o.getId().toString());
    ...
    private void kill() {
        ...
        for (Dataset dataset : PiqnicClient.nodeInstance.getDatasets()) {
            for (BaseFragmentBase fragment : dataset.getFragments()) {
        ...
        for (FragmentBase fragment : PiqnicClient.nodeInstance.getAllFragments()) {
            if (fragment.ownedBy(this)) {
                ...
                try {
                    d.addFragment(newFragment.toBaseFragment(p.addFragmentInit(newFragment, triples, 1)));
                } catch (IOException e) {
                }
            }
        }
    ...
    @Override
    public Set<MetaFragmentBase> getMetaFragments(int ttl, UUID id) throws IOException {
        Socket socket;
        try {
            socket = new Socket(address, port);
        } catch (Exception e) {
        ...
        String response;
        while ((response = in.readLine()) != null) {
            ret.add(new Gson().fromJson(response, PredicateSpecificMetaFragment.class));
        }
    ...
    public Map<IndexPeer, Set<MetaFragmentBase>> getMetaFragmentsOrdered(int ttl, UUID id) throws IOException {
        Socket socket;
        try {
            socket = new Socket(address, port);
        } catch (Exception e) {
        ...
        Set<Tuple<String, String>> contained = new HashSet<>();
        Map<IndexPeer, Set<MetaFragmentBase>> ret = new HashMap<>();
        String response;
        while ((response = in.readLine()) != null) {
            MetaFragmentBase mfb = new Gson().fromJson(response, PredicateSpecificMetaFragment.class);
            if(contained.contains(new Tuple<>(mfb.getBaseUri(), mfb.getId()))) continue;
            contained.add(new Tuple<>(mfb.getBaseUri(), mfb.getId()));
            if(ret.containsKey((IndexPeer)mfb.getNode())) {
                ret.get((IndexPeer)mfb.getNode()).add(mfb);
            } else {
                Set<MetaFragmentBase> set = new HashSet<>();
                set.add(mfb);
                ret.put((IndexPeer)mfb.getNode(), set);
            }
        }
    ...
    @Override
    public IBloomFilter<String> getConstituentsForFragment(MetaFragmentBase fragment) throws IOException {
        Socket socket;
        try {
            socket = new Socket(address, port);
        } catch (Exception e) {
        ...
        out.println("B");
        out.println(fragment.getBaseUri());
        out.println(fragment.getId());
        ...
        if(str.equals("")) return PrefixPartitionedBloomFilter.empty();

        PrefixPartitionedBloomFilter f = PrefixPartitionedBloomFilter.fromString(str);
        socket.close();
        return f;
    }
