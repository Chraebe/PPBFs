...
public class CLI implements IClient {
    ...
    private void processCommand(String line) throws IOException {
        if (line.equals(":q") || line.equals(":quit")) quit();
        ...
        else if (line.startsWith(":i")) locational();
        else if (line.startsWith(":g")) summary();
        else if (line.startsWith(":p")) match();
        else if (line.startsWith(":b1")) {
            String[] words = line.split(" ");
            bloom(words[1], words[2]);
        }
        else if (line.startsWith(":b2")) {
            String[] words = line.split(" ");
            bloomi(words[1], words[2], words[3]);
        }
            ...
            String[] words = line.split(" ");
            recalc(words[1], words[2]);
        }
        else help();
    }

    private void recalc(String baseUri, String id) {
    ...
    private void b3(String test) {
        PrefixPartitionedBloomFilter bloom = PrefixPartitionedBloomFilter.empty();
        bloom.showPrefix(test);
    }

    private void bloom(String pred, String test) {
        PrintWriter writer = ((ClientThread) Thread.currentThread()).writer;
        for(FragmentBase f : PiqnicClient.nodeInstance.getAllFragments()) {
            if(f.getId().equals(pred)) {
                writer.println(f.getBloom().mightContain(test));
            }
        }
    }

    private void bloomi(String pred1, String pred2, String test) {
        PrintWriter writer = ((ClientThread) Thread.currentThread()).writer;
        FragmentBase f1 = null;
        FragmentBase f2 = null;
        for(FragmentBase f : PiqnicClient.nodeInstance.getAllFragments()) {
            if(f.getId().equals(pred1)) {
                f1 = f;
            }
            if(f.getId().equals(pred2)) {
                f2 = f;
            }
        }
        if((f1 == null) || (f2 == null)) {
            writer.println("Fragment not found");
            return;
        }

        IBloomFilter<String> b = f1.getBloom().intersect(f2.getBloom());
        writer.println(b.mightContain(test));
    }

    private void locational() {
        PrintWriter writer = ((ClientThread) Thread.currentThread()).writer;
        PiqnicClient.nodeInstance.indexLocationalIndex();
        writer.println("Locational Index computed");
    }

    private void summary() {
        PrintWriter writer = ((ClientThread) Thread.currentThread()).writer;
        PiqnicClient.nodeInstance.indexSummaryGraph();
        writer.println("Summary Graph computed");
    }
    ...
    private void match() {
        ...
        PrintWriter writer = ((ClientThread) Thread.currentThread()).writer;
        PpbfIndex sg = PiqnicClient.nodeInstance.getPpbfIndex();
        writer.println(sg.toString());
    }
    ...
    private void createDataset(HDT hdt) throws IOException {
        ...
        for (int i = 1; i < predicateDictionary.getNumberOfElements() + 1; i++) {
            ...
            BaseFragmentBase base = fragment.toBaseFragment(peers);
    ...
    public void queryNetwork(String sparql) throws IOException {
        ...
        final PiqnicExtGraph graph;
        graph = new PiqnicExtGraph();
